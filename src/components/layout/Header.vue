    ...
    <MDBNavbar expand="lg" light bg="white" position="top" container>
      ...
      <MDBCollapse v-model="collapse" id="sidenav">
        <MDBNavbarNav right class="ms-auto ">
          ...
          <MDBNavbarItem class="mx-2" :to="{name: 'dao-list', query: {}}" :title="t('organizations')"><i class="bi bi-people"/> <span class="fw-bold">{{ t('organizations') }}</span></MDBNavbarItem>
          
          <MDBNavbarItem class="mx-2" :to="{name: 'market', params: {id: daoId}, query: {}}" :title="t('market')"><i class="bi bi-bag"/> <span class="fw-bold">{{ t('market') }}</span></MDBNavbarItem>
          ...
          <MDBNavbarItem  v-if="isSignedIn" class="mx-2">
            ...
            <MDBDropdown class="nav-item" align="end" v-model="dropdown">
              ...
              <MDBDropdownMenu aria-labelledby="dropdownMenuButton">
                <MDBDropdownItem :href="config.near.walletUrl">{{t('wallet')}}</MDBDropdownItem>
                <MDBDropdownItem tag="button" @click="logout()">{{t('log_out') }}</MDBDropdownItem>
              </MDBDropdownMenu>
            </MDBDropdown>
          </MDBNavbarItem>
        </MDBNavbarNav>
          <MDBBtn v-if="isSignedIn" @click="createDaoPage()"  color="primary" rounded class="bg-gradient-100 fs-6 text-uppercase mx-2"><i class="bi bi-plus me-1"/>{{ t('create_dao') }}</MDBBtn>
          <MDBBtn v-else @click="login()" color="primary" rounded class="bg-gradient-100 fs-6 text-uppercase mx-2" data-mdb-toggle="tooltip" data-mdb-placement="bottom" title="Log In">{{ t('log_in') }}</MDBBtn>
      </MDBCollapse>
    </MDBNavbar>
  </header>
</template>
