...
<InputNumber :labelName="`NEAR ${t('amount')}`" :balance="dao.treasury.near" :max="dao.treasury.near" id="near_amount" addon="NEAR"/>
<InputNumber :labelName="`Token ${t('amount')}`" :balance="dao.treasury.token.free" :max="dao.treasury.token.free" id="token_amount" :addon="dao.treasury.token.meta.symbol"/>
   <!-- <div class="text-start">
      <MDBSwitch :label="`${t('lock')} NEAR`" v-model="nearCollapse"/>
      ...
      <MDBSwitch :label="`${t('lock')} Token`" v-model="tokenCollapse"/>
