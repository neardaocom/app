...
export default class ProposalVotingTransformer implements TransformerInterface {
    ...
    transform(value: any): ProposalVoting {
        ...
        const trans = {
            id: value.id,
            code: template.code,
            title: this.t('wf_templ_' + template.code + '_v' + template.version + '_s' + value.workflowScenarioId + '_title', args),
            description: DocsHelper.getProposalDescription(this.docs, value.id),
            typeCode: template.code,
            type: this.t('wf_templ_' + template.code + '_v' + template.version + '_s' + value.workflowScenarioId),
            stateCode: value.state,
            state: this.t("proposal_state_" + value.state),
            ...
            choice: (choiceIndex) ? this.t("vote_type_" + choiceIndex) : null,
            progress: progress,
